...
/**
 * IMPROVED: Timezone Formatter Helper
 * Added safety checks for null/undefined strings to prevent crashes.
 */
const getFormatters = (language = 'en-US', timeZone = 'UTC') => {
    
    let validTimeZone = timeZone;
    
    // IMPROVED: Handle string "null"/"undefined" often sent by frontends
    if (!validTimeZone || validTimeZone === 'null' || validTimeZone === 'undefined') {
        validTimeZone = 'UTC';
    }
...
const generateCommonSlotOptions = async (slotsData = [], userId, flowId, language, timezone) => {
    ...
    for (const dateItem of slotsData) {
        ...
        for (const s of dateItem.slots) {
            ...
            while (currentCalcMs + intervalMs <= endMs) {
                // IMPROVED: Ensure the slot ENDS in the future, not just starts
                if (currentCalcMs > now && (currentCalcMs + intervalMs) > now) {
                    totalCapacity++;
                    hasFutureSlots = true;
                }
                currentCalcMs += intervalMs + bufferMs;
            ...
            // FIX: Formats the date using the USER'S timezone, ensuring rollover (midnight) is handled correctly
            const slotSpecificDate = formatDate.format(start);
    ...
    for (const slotData of calculatedSlots) {
        ...
        const availableSlots = Math.max(0, totalCapacity - slotRecordCount);
        // ID remains absolute (startMs/endMs) so it's safe across timezones
        const uniqueId = `PRE-S_${s.id}_${startMs}_${endMs}_${s.interval}_${s.buffer || 0}`;
...
const generateSubSlotOptions = async (commonSlotId, nodeCount, userId, flowId, language, timezone) => {
    ...
    // IMPROVED: Robust ID Parsing logic
    // This prevents crashes if IDs contain hyphens or underscores
    bookedArray.forEach(slotStr => {
        if (!slotStr) return;
        
        // Structure: PRE-SUB_DisplayStart_DisplayEnd_Timestamp_SlotID-NodeCount
        const parts = slotStr.split('_');
        
        // Ensure it's a sub-slot and has enough parts
        if (parts.length >= 5 && parts[0] === 'PRE-SUB') {
            const timestamp = parts[3]; 
            
            // Safer logic to extract SlotID:
            // Join everything after timestamp, then strip the NodeCount suffix
            const remaining = parts.slice(4).join('_');
            const lastHyphenIndex = remaining.lastIndexOf('-');
            
            const slotIdPart = lastHyphenIndex !== -1 
                ? remaining.substring(0, lastHyphenIndex) 
                : remaining;
    ...
    const { formatTimeShort, formatTimeLong } = getFormatters(language, timezone);
    const safeNow = Date.now() + 60000; // 1 minute safety buffer
    ...
    while (currentMs + intervalMs <= endMs) {
        ...
        // IMPROVED: Strict check to ensure we don't list slots in the past (Ghost Booking Prevention)
        if (currentMs <= safeNow) {
            currentMs += intervalMs + bufferMs;
            continue; 
        }
        ...
        // console.log("Checking Lock Key:", currentLockKey, 'locked:', lockedTimestamps.has(currentLockKey));
        ...
        // Display formatted to User's Timezone
        const displayStart = formatTimeShort.format(dateStart);
        const displayEnd = formatTimeShort.format(dateEnd);
        
        // ID contains Absolute Timestamp (currentMs) -> Timezone Safe
        const uniqueSubId = `PRE-SUB_${displayStart}_${displayEnd}_${currentMs}_${SlotId}-${nodeCount}`;
...
const generateDynamicPrompt = async (
    ...
    try {
        ...
        // IMPROVED: Initialize formatters with the dynamic timezone passed from frontend
        // This ensures all date calculations below respect the user's view
        const { formatDate, formatTimeShort } = getFormatters(language, timezone);
