...
//             if (subSlotCount === 0) continue;

//             const uniqueId = `PRE-S_${s.id}_${startMs}_${endMs}_${s.interval}_${s.buffer || 0}_${s.id}`;

//             result.push({
//                 id: uniqueId,
//                 value: `${formattedDate}: ${formatTimeShort(start)}-${formatTimeShort(end)}`,
//                 description: `${formatTime(start)} - ${formatTime(end)} (${subSlotCount} Slots)`, 
//                 type: 'List',
...
//     const SlotId = parseInt(parts[6] || '0', 10);
...
//         const uniqueSubId = `PRE-SUB_${parts[1]}_${displayStart}_${displayEnd}_${currentMs}_${SlotId}-${nodeCount}`;
...
// ) => {
//     try {
//         if (!flowTrainingData?.nodes?.length) return { reply: "Sory, Missing my Flow", done: true };
...
const generateDynamicPrompt = async (
    ...
    try {
        if (!flowTrainingData?.nodes?.length) return { reply: "Sory, Missing my Flow", done: true };
        const WELCOME_MESSAGE = `Hi ${profileName}! I'm Eva. It’s a pleasure to assist you with your booking today. Let's get started.`;
        const { nodeMap, edgeMap, rootNodeId } = buildFlowMaps(flowTrainingData.nodes, flowTrainingData.edges);
        let state = { currentNodeId: rootNodeId, collected: {}, retryCount: 0, tempSlotSelection: null, inputIndex: 0 };
        let isNewSession = true;
