...
// IMPROVED: Removed global 'browserInstance'. 
// In Cloud Run/Serverless, we must launch a new browser for every request 
// because the CPU is throttled when the request ends, which kills the global browser connection.
...
const generateAndSendBookingSlip = async (userPhone, bookingDetails) => {
    // IMPROVED: Initialize variables outside try/catch for proper cleanup in finally block
    let browser = null;
    let page = null;
    ...
    try {
        ...
        // IMPROVED: Launch browser INSIDE the request
        // This prevents 'TargetCloseError' because we get a fresh process every time
        ...
        browser = await puppeteer.launch({ 
            ...
            args: [
                ...
                '--disable-gpu',
                '--font-render-hinting=none',
                // IMPROVED: Added extra flags for stability in container environments
        ...
        // IMPROVED: Added timeout to prevent infinite hanging
        // 'networkidle0' is good for complex CSS, but can sometimes hang if a network request is pending.
        ...
        const contentHeight = await page.evaluate(() => {
            const container = document.getElementById('main-container');
            // IMPROVED: Fallback logic for height calculation
            if (container) return container.offsetHeight;
            return document.body.scrollHeight;
        });

        // IMPROVED: Ensure minimum height to avoid zero-height PDF errors
        const finalHeight = contentHeight > 0 ? contentHeight : 800;
        ...
        // IMPROVED: CRITICAL - Always close resources to prevent memory leaks in the container
        if (page) {
