...
// const PROMPT_TRIM_THRESHOLD = 40000;
// const OPTION_KEEP_COUNT = 12; 

// const estimatePromptSize = (str = '') => (typeof str === 'string' ? str.length : JSON.stringify(str).length);
// const compactHugeOptions = (promptText, keep = OPTION_KEEP_COUNT) => {
//     try {
...
//                         const trimmed = arr.slice(0, keep);
//                         trimmed.push({ id: 'P-MORE', value: 'More options available...' });
//                         return JSON.stringify(trimmed);
//                     }
//                     return match;
//                 } catch (err) {
//                     if (match.length > 2000) {
//                         const snippet = match.slice(0, 2000);
//                         return snippet + ',"P-MORE":"More options available..."]';
//                     }
//                     return match;
//                 }
//             }
...
//             const conversationText = session.conversation.map(c => `${c.sender}: ${c.message}`);
//             const generatedPrompt = await generateDynamicPrompt(conversationText, userPrompt, flowTrainingData);
//             let promptToSend = typeof generatedPrompt === 'string' ? generatedPrompt : JSON.stringify(generatedPrompt);

//             if (estimatePromptSize(promptToSend) > PROMPT_TRIM_THRESHOLD) {
//                 promptToSend = compactHugeOptions(promptToSend, OPTION_KEEP_COUNT);
//             }
//             let aiResponse = await generateAIResponse(promptToSend, userPhone, clearUserSessionData, resetUserInput);
//             console.log(aiResponse,'aiResponseaiResponse')
//             const optionsData = safeParseOptions(aiResponse);
//             const isValid = !!(optionsData?.message && (/(^|\s|,)\s*FI-\d{13,}(\b|,|$)/.test(optionsData.message) || true));
//             updateConversationHistory(userPhone, userPrompt, aiResponse);
//             session = userConversationHistories.get(userPhone);
//             const cleanAIResp = cleanAIResponse(aiResponse);
//             const extractJsonFromResp = extractJsonFromResponse(aiResponse);
...
//                 isValid
//             ) {
//                 const { message = "", options = [] } = optionsData;

//                 const messageParts = typeof message === "string"
//                     ? message.split(",").map(p => p.trim()).filter(Boolean)
//                     : [];


//                 if (messageParts.length) {
//                     const fileIds = messageParts.filter(p => p.startsWith("FI-"));
//                     let matchedFiles = [];

//                     if (fileIds.length) {
//                         matchedFiles = await ChatBotModel.aggregate([
//                             {
//                                 $match: {
...
//                 const result = { isQuestion: true };

//                 if (orderedOutputs.length) {
//                     result.message = orderedOutputs;
//                 }
...
//                         mainTitle,
//                         type: type?.toLowerCase() || "",
//                         resp: (rest.length ? rest : [{ id: firstId, value: mainTitle }]).map(({ id, value }) => ({
//                             _id: id,
//                             title: value,
//                         })),
//                     };
...
//             try {
//                 if (extractJsonFromResp) {
//                     console.log(extractJsonFromResp,'extractJsonFromRespsssssssssssssssssssssssssssss')
//                     const history = parseChatHistory(session.conversation);
//                     const sentimentScores = await getFinalSentimentScore(history, userPhone, userId);
//                     const currentScores = fillMissingSentimentFields(sentimentScores[userPhone] || {});
//                     const appointmentUid = new mongoose.Types.ObjectId();
